...
def pick(paragraphs, select, k):
    ...
    for p in paragraphs:
      if select(p):
        select_paragraphs.append(p)
    if k > len(select_paragraphs) - 1:
      return ''
    else:
...
def about(subject):
    ...
    def select(paragraph):
      string = remove_punctuation(paragraph)
      string = split(lower(string))
      for word in subject:
        if word in string:
          return True
      return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    word_diff = [diff_function(typed_word, word, limit) for word in word_list]
    min_len = min(word_diff)
    if (typed_word in word_list) or (min_len > limit):
      return typed_word
    else:
      min_index = word_diff.index(min_len)
